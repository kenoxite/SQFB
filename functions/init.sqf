                ...
                if (SQFB_opt_showSquad) then {
                    ...
                    for "_i" from 0 to (count _SQFB_units) -1 do
                    {
                        if ((typeName (_SQFB_units select _i))!="STRING") then {
                            ...
                            private _maxRange = if (_isPlayerAir) then { SQFB_opt_maxRangeAir } else { SQFB_opt_maxRange };
                            // Skip units outside the max
                            if (_dist2D > _maxRange) then { continue };
                            ...
                            if ((_SQFB_showHUD || (!_SQFB_showHUD && _SQFB_opt_AlwaysShowCritical)) && _canSee >= 0.2) then {
                                if (_alive || (!_alive && _SQFB_opt_showDead && (_unit getVariable "SQFB_veh") == _unit)) then {
                                    ...
                                    if (_isPlayerAir) then {
                                        if (_isOnFoot) then {
                                            _iconSize = ((linearConversion[ 0, _maxRange min 100, _dist2D, (1.8 * 2) * _zoom, 0.1, true ])) min 1.8;

                                        } else {
                                            _iconSize = ((linearConversion[ 0, _maxRange min 100, _dist2D, (2 * 2) * _zoom, 0.3, true ])) min 2;
                                        };
                                    } else {
                                        if (_isOnFoot) then {
                                            _iconSize = ((linearConversion[ 0, _maxRange min 200, _dist2D, (1.8 * 2) * _zoom, 0.1, true ])) min 1.8;

                                        } else {
                                            _iconSize = ((linearConversion[ 0, _maxRange min 200, _dist2D, (1.8 * 2) * _zoom, 0.3, true ])) min 1.8;
                                        };
                                    };
                                    ...
                                    if (_SQFB_opt_scaleText) then {
                                        if (_isPlayerAir) then {
                                            if (_isOnFoot) then {
                                                _text_size = ((linearConversion[ 0, _maxRange min 200, _dist2D, (0.04 * 2) * _zoom, 0.02, true ])) min 0.04;
                                            ...
                                            if (_isOnFoot) then {
                                                ...
                                                _text_size = ((linearConversion[ 0, _maxRange min 200, _dist2D, (0.052 * 2) * _zoom, 0.02, true ])) min 0.052;
                                            };
                                        };
                                    } else {
                                        _text_size = 0.03;
                ...
                if (SQFB_opt_AlwaysShowEnemies || (SQFB_opt_showEnemies && time >= SQFB_showEnemiesMinTime)) then {
                    for "_i" from 0 to (count _SQFB_knownEnemies) -1 do
                    {
                        if ((typeName (_SQFB_knownEnemies select _i))!="STRING" && (_SQFB_showHUD || SQFB_opt_AlwaysShowEnemies)) then {
                            ...
                            private _alive = alive _unit || damage _unit < 1;
                            // Skip if dead
                            if (!_alive) then { continue };

                            private _veh = vehicle _unit;
                            private _vehPlayer = vehicle player;
                            private _isPlayerAir = if ((typeOf _vehPlayer) isKindOf "Air") then { true } else { false };
                            private _dist2D = _vehPlayer distance2D _unit;
                            private _maxRange = if (_isPlayerAir) then { SQFB_opt_showEnemiesMaxRangeAir } else { SQFB_opt_showEnemiesMaxRange };
                            // Skip units outside the max range
                            if (_dist2D > _maxRange) then { continue };

                            private _isOnFoot = (typeOf _veh isKindOf "Man");
                            private _canSee = if (!_isOnFoot) then { [objNull, "VIEW"] checkVisibility [eyePos player, AtlToAsl(_unit modeltoworld [0,0,0])] } else { [objNull, "VIEW"] checkVisibility [eyePos player, eyePos _unit] };
                            ...
                            if (_enemy == _unit || (!isNull _enemyTagger && (_enemy == _enemyTagger && _enTagFirstTime))) then {
                                _enemyTagger setVehiclePosition [position _veh, [], 0, "CAN_COLLIDE"];
                            ...
                            // Skip if close
                            private _minRange = if (_isPlayerAir) then { SQFB_opt_showEnemiesMinRangeAir } else { SQFB_opt_showEnemiesMinRange };
                            if (_minRange > 0 && {_dist2D <= _minRange}) then { continue };
                            ...
                            if (_isPlayerAir) then {
                                if (_isOnFoot) then {
                                    _iconSize = ((linearConversion[ 0, _maxRange min 100, _dist2D, (0.6 * 2) * _zoom, 0.3, true ])) min 0.6;

                                } else {
                                    _iconSize = ((linearConversion[ 0, _maxRange min 100, _dist2D, (1 * 2) * _zoom, 0.5, true ])) min 1;
                                };
                                _text_size = 0.03;
                            } else {
                                if (_isOnFoot) then {
                            ...
                            if (!_isPlayerAir && _SQFB_opt_showText && _text_size > 0.02) then {
                                if (SQFB_opt_showDistEnemy) then { _text = format ["%1m", round _dist2D] };
                            };
